...
public class Action   {
    ...
    public static void mouseHoverByJavaScript(WebElement ele) {
        ...
        WebElement elementToHover = DriverManager.getInstance().Driver.findElement(By.id("yourElementId"));

        // Create a JavascriptExecutor instance
        JavascriptExecutor jsExecutor = (JavascriptExecutor) DriverManager.getInstance().Driver;

        // Use JavaScript to trigger the mouse hover event on the element
        String script = "var element = arguments[0];"
                + "var mouseEvent = document.createEvent('MouseEvents');"
                + "mouseEvent.initEvent('mouseover', true, true);"
                + "element.dispatchEvent(mouseEvent);";
        jsExecutor.executeScript(script, elementToHover);

        // After hovering, you can perform additional actions if needed
        // For example, click a sub-element that appears after the hover
    ...
    public static boolean launchUrl(WebDriver driver,String object) {
        boolean flag = false;
        try {
            ...
            if (flag) {
                System.out.println("Successfully launched \""+object+"\"");
            } else {
                System.out.println("Failed to launch \""+object+"\"");
            }
        }
    }
    ...
    public static void implicitWait(WebDriver driver, Integer timeOut) {
